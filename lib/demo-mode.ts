...
import menuPackagesData from '@/data/mock/packages.json'
import eventTypesData from '@/data/mock/events.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'packages.json': menuPackagesData,
  'events.json': eventTypesData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetMenuPackages') || query.includes('nodeMenuPackages')) {
      return loadMockData('packages.json')
    }

    if (query.includes('GetEventTypes') || query.includes('nodeEventTypes')) {
