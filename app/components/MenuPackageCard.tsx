...
interface MenuPackageCardProps {
  item: DrupalMenuPackage
}

export default function MenuPackageCard({ item }: MenuPackageCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6 relative">
        <div className="absolute -top-6 left-6 w-12 h-12 rounded-full bg-primary-100 border-4 border-white flex items-center justify-center shadow-sm">
          <UtensilsCrossed className="w-5 h-5 text-primary-700" />
        </div>
        <div className="pt-4">
          {(item as any).pricePerPerson && (
            <p className="text-sm text-primary-700 font-medium mb-2">{(item as any).pricePerPerson}</p>
          )}
